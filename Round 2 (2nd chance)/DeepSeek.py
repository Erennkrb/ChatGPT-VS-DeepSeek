...
# Load sounds (optional, requires `winsound` on Windows or `playsound` library)
try:
    import winsound
except ImportError:
    winsound = None
...
# Draw Bullets
def draw_bullet(x, y):
    canvas.create_rectangle(x, y, x + BULLET_SIZE, y + BULLET_SIZE, fill="yellow", tags="bullet")

# Draw Asteroids
def draw_asteroid(x, y):
    canvas.create_text(x, y, text="🪨", font=("Arial", ASTEROID_SIZE), tags="asteroid")
...
def game_loop():
    ...
    for asteroid in asteroids[:]:
        ...
        if not coords:  # Asteroid no longer exists (e.g., deleted due to collision)
    ...
    for bullet in bullets[:]:
        ...
        if not bullet_coords:  # Bullet no longer exists
            continue
        for asteroid in asteroids[:]:
            asteroid_coords = canvas.coords(asteroid)
            if not asteroid_coords:  # Asteroid no longer exists
                continue
            if abs(bullet_coords[0] - asteroid_coords[0]) < ASTEROID_SIZE and abs(bullet_coords[1] - asteroid_coords[1]) < ASTEROID_SIZE:
                canvas.delete(bullet)
                canvas.delete(asteroid)
                bullets.remove(bullet)
                asteroids.remove(asteroid)
                play_sound("SystemAsterisk")  # Play sound
                score += 10
                update_score()
...
def handle_key(event):
    ...
    if key == "s":
        ...
        x, y = canvas.coords("player")
        draw_bullet(x, y - PLAYER_SIZE)
        bullets.append((x, y - PLAYER_SIZE))
        play_sound("SystemExclamation")  # Play sound
